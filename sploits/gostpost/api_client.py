...
class ApiClient:
    ...
    @staticmethod
    async def register(base_url, name, passphrase) -> "ApiClient | str":
        cookie_jar = CookieJar(unsafe=True)
        sess = ClientSession(cookie_jar=cookie_jar)
        try:
            async with sess.post(
    ...
    async def send_letter(self, recipient_name, content) -> None:
        recipient = await self.get_boyarin(name=recipient_name)
        if recipient is None:
            return False
        data = {
            "recipient": recipient.id,
            "content": await self.encrypt_letter(recipient, content),
        }
        async with self.session.post(self.base_url + "/letters/send", json=data) as r:
            return r.status == 200

    async def get_shared_key(self, recipient: Boyarin) -> bytes:
        cache_key = recipient.id
        if cache_key in self.cache:
            return self.cache[cache_key]
        data = {"other_id": recipient.id}
